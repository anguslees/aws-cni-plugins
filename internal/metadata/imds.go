...
func (imds TypedIMDS) getValue(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		// NB: Error is wrapped, so IsNotFound can find it
		return "", fmt.Errorf("error retrieving %s: %w", key, err)
	}
	return data, nil
}

func (imds TypedIMDS) getList(ctx context.Context, key string) ([]string, error) {
	data, err := imds.getValue(ctx, key)
	if err != nil {
		return nil, err
	}
...
func (imds TypedIMDS) GetInstanceID(ctx context.Context) (string, error) {
	return imds.getValue(ctx, "instance-id")
}
